...
def main(_):
  with tf.Graph().as_default():
    ...
    ##############################################################
    pathTensor = tf.placeholder(tf.string,name='input')
    ...
    score = tf.nn.softmax(logits,name='score')
    saver = tf.train.Saver()
    sess = tf.Session()
    saver.restore(sess, FLAGS.checkpoint_path)
    from tensorflow.python.framework import graph_util
    constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ['score'])
    with tf.gfile.FastGFile('code/inv4_640.pb', mode='wb') as f:
