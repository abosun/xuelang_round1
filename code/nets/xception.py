...
def attention(net, f):
    with slim.arg_scope([slim.conv2d], 
              ...
              weights_regularizer=slim.l2_regularizer(0.005),scope='atten'):
        B = slim.conv2d(net, 50, [1, 1],activation_fn=tf.nn.tanh, scope='atten1')
        B = slim.conv2d(B, 1, [1,1], scope='atten2')
        B = tf.nn.softmax(tf.reshape(B, [-1, B.get_shape()[1]* B.get_shape()[2]]))
        B = tf.expand_dims(B,-1)        
        X = tf.reshape(net, [-1, net.get_shape()[1]*net.get_shape()[2],f])
        Z = tf.matmul(tf.transpose(X, perm=[0,2,1]),B)
        Z = tf.reshape(Z, [-1,1,1,f], name='features')
    return Z
  ...
  with tf.variable_scope(
    ...
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        net = end_points['xception_41/exit_flow/block1/unit_1/xception_module/separable_conv2_pointwise']
        if global_pool:
          # Global average pooling.
          net = tf.reduce_mean(net, [1, 2], name='global_pool', keepdims=True)
          #net = slim.max_pool2d(net, net.get_shape()[1:3], stride=net.get_shape()[1:3], padding='SAME')
          #net = attention(end_points['xception_41/exit_flow/block1/unit_1/xception_module/separable_conv2_pointwise'], 1024)
        ...
        if num_classes:
          ...
          net = tf.reduce_max(net, 1,keepdims=True)
